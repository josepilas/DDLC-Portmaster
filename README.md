# DDLC ARM Linux Compatibility Wrapper

<img src="https://i.ibb.co/S7BR4yyS/186106.webp" width="400" height="400" alt="Imagem">

Unofficial, asset-free ARM Linux wrapper for running a user-owned copy of
...
- `DDLC.sh` ARM Linux launcher.
- ARM Linux metadata inside `ddlc/`: `port.json`, `gameinfo.xml`, `screenshot.png`, and `ddlc.md`.
...
The ARM Linux release ZIP for this package is `ddlc.zip`.
...
```text
ddlc/logs/log.txt
ddlc/logs/ddlc-arm-linux.log
```

`log.txt` appends one short crash entry per failed launch, including the exact
date/time, exit code, and error summary. `ddlc-arm-linux.log` is replaced each
...
- Missing PortMaster Westonpack runtime. The ARM Linux package declares
